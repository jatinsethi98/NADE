...
| 62 | 2a/1f | **+** a draft card names its **recipients**, and flags one this mailbox has never written to | `backend/testdata/injection/README.md` finding 10: `identity-02` and `tool-01`/`tool-06` are contained **only** if the card shows the actual recipient list and the `never_messaged` flag — "an approval card that renders only the body launders a redirected draft". The body is prose a model wrote after reading somebody else's email; the address list is not. The contract has carried `data.to` and `data.never_messaged` since P1 and nothing rendered them |
| 63 | 2a | **+** the feed rows are what the *server* paged, and the load-more sentinel is real | not a visual change: `DESIGN.md` §2a always described a scrolling feed, and `GET /feed` was always cursored. Recorded because the DEBUG fixture world now pages too (four cards, then two), which is what makes the sentinel reachable by a test at all |
| 64 | 2a | the outbound-verb sweep screens **send / forward / archive**, not `delete` | §4 forbids "no sending, no archiving, no Gmail mutation". Deleting an *agent* is none of those and is a real v1 control (`DELETE /agents/{id}`), and `Sent` is a Gmail mailbox — a place, not a promise, exempted by its identifier rather than by weakening the pattern. The **server's** card-body screen is broader, because there the string is a claim about what happened to somebody's mail rather than the name of a control |
