...
pub async fn gmail(
    ...
    let Some(account) = state.account().await? else {
        tracing::info!("a push arrived before any account was connected");
        return Ok(StatusCode::NO_CONTENT);
    };

    // A valid delivery for another watch on the same topic must not drive this
    // account's cursor.
    let addressed = &delivery.notification.email_address;
    if !addressed.eq_ignore_ascii_case(&account.email) {
        tracing::warn!(%addressed, "a push for a different mailbox was ignored");
        store::audit(
            &state.pool,
            account.id,
            "gmail_push_wrong_mailbox",
    ...
    let dedupe = format!("{}:{}", incremental::KIND, account.id);
    let enqueued = queue
        .enqueue_unique(
            incremental::KIND,
            &json!({
    ...
    if enqueued.is_none() {
        ...
        store::request_rerun(&state.pool, account.id).await?;
    }
